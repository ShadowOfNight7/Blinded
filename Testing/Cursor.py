...
def initialize(repetitions=1):
    ...
    for i in charsize:
        ...
        chary += i[1]
    pygetwindow.getActiveWindow().size = (
        maximized_screen_rect[2],
        maximized_screen_rect[3],
    )
...
def get_mouse_coords(charactersize, mouse_coords: tuple | None = None, get_text=False):
    rect = RECT()
    ...
    if mouse_coords == None:
        mouse_coords = (
            win32gui.ScreenToClient(hwnd, mouse.get_position())[0]
            + ctypes.windll.user32.GetSystemMetrics(5) * 2,
            win32gui.ScreenToClient(hwnd, mouse.get_position())[1]
    ...
    if is_full_screen() == True:
        return (
            math.floor(mouse_coords[0] / charactersize[0]) - 1,
            math.floor(mouse_coords[1] / charactersize[1]) + 2,
        )
    elif is_full_screen() == "maximized":
        return (
            math.floor(mouse_coords[0] / charactersize[0]) - 1,
            math.floor(mouse_coords[1] / charactersize[1] + 0.5),
        )
    else:
        return (
            math.floor(mouse_coords[0] / charactersize[0]) - 1,
            math.floor(mouse_coords[1] / charactersize[1]),
        )
...
def get_pixel_coords(charactersize, character_mouse_coords):
    ...
    if is_full_screen() == True:
        mouse_coords = (
            (character_mouse_coords[0] + 1 + 0.5) * charactersize[0],
            (character_mouse_coords[1] - 2 + 0.5) * charactersize[1],
        )
    elif is_full_screen() == "maximized":
        mouse_coords = (
            (character_mouse_coords[0] + 1 + 0.5) * charactersize[0],
            (character_mouse_coords[1] - 0.5 + 0.5) * charactersize[1],
        )
    else:
        mouse_coords = (
            (character_mouse_coords[0] + 1 + 0.5) * charactersize[0],
            (character_mouse_coords[1] + 0.5) * charactersize[1],
        )

    return win32gui.ClientToScreen(
        hwnd,
        (
            round(mouse_coords[0] - ctypes.windll.user32.GetSystemMetrics(5) * 2),
            round(mouse_coords[1] + ctypes.windll.user32.GetSystemMetrics(4) * 2),
        ),
    )
