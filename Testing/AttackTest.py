...
def AddAttackAim(Range: tuple, Duration: int, Clickable=True):
    ...
    CreateButton(
        (Coords[0] + 2, Coords[1] + 1), (Coords[0] - 2, Coords[1] - 1), MainClock
    )
    MainClock += 1
...
def AttackAim():
    score = 0
    for i in AimTarget:
        i["Coords"] = pyterm.addItem(
            assets[i["Phase"]], i["X"], i["Y"], "center", "center"
        )
        i["CurrentDuration"] -= 1
        if (CheckButton(i["Id"], False) and (i["Clickable"] == False)) or (
            CheckButton(i["Id"]) and i["Clickable"]
        ):
            AimTarget.remove(i)
            RemoveButton(i["Id"])
...
def CheckButton(ButtonId, Click=True) -> bool:
    ...
    for i in buttons:
        if i["Id"] == ButtonId:
            if (
                (i["Point1"][0] >= location[0] >= i["Point2"][0])
                or (i["Point1"][0] <= location[0] <= i["Point2"][0])
            ) and (
                (i["Point1"][1] >= location[1] >= i["Point2"][1])
                or (i["Point1"][1] <= location[1] <= i["Point2"][1])
            ):
                if Click:
...
def CircleStay(Range: tuple, Duration: int, Speed: float, Unpredictability: int):
    ...
    if circleStay["Time"] == 0:
        ...
        circleStay["location"] = (
            random.randint(-Range[0], Range[0]),
            random.randint(-Range[1], Range[1]),
        )
        circleStay["rotation"] = random.randint(0, 359)
    if random.randint(1, Unpredictability) == 1:
        circleStay["rotation"] = random.randint(0, 359)
    if not (
        (Range[0] >= circleStay["location"][0] >= -Range[0])
        and (Range[1] >= circleStay["location"][1] >= -Range[1])
    ):
        circleStay["location"] = (
            circleStay["location"][0] / 1.025,
            circleStay["location"][1] / 1.025,
        )
        circleStay["rotation"] = random.randint(0, 359)
    circleStay["location"] = (
        circleStay["location"][0]
        + math.cos(math.radians(circleStay["rotation"])) * Speed,
        circleStay["location"][1]
        + math.sin(math.radians(circleStay["rotation"])) * Speed,
    )
    circleStay["absoluteloc"] = pyterm.addItem(
        assets["CircleStay"],
        round(circleStay["location"][0]),
        round(circleStay["location"][1]),
        "center",
        "center",
    )

    if (
        (circleStay["absoluteloc"][0] + 3.5)
        >= location[0]
        >= (circleStay["absoluteloc"][0] - 3.5)
    ) and (
        (circleStay["absoluteloc"][1] + 3.5)
        >= location[1]
        >= (circleStay["absoluteloc"][1] - 3.5)
    ):
        score += 0.0925
...
def AimMinigame(Speedrange: tuple, Repeats=1):
    ...
    if aimMinigame["Repeats"] == 0:
        ...
        return False
    elif (aimMinigame["CurrentFrame"] >= 11) and (
        aimMinigame["CurrentWait"] >= aimMinigame["Speed"]
    ):
    ...
    pyterm.addItem(
        assets["AimMinigame"].splitlines()[aimMinigame["CurrentFrame"] - 1],
        0,
        7,
        "center",
        "center",
    ...
    if (
        MouseDetect.ClickDetect("Left", "On") or MouseDetect.GeneralDetect(32, "On")
    ) and (clickbuffer == False):
...
def KeyboardMinigame(Inputs: list, Speed: int, Time: int):
    ...
    pyterm.addItem(
        "Press " + str(keyboardMinigame["CurrentKey"]), 0, 7, "center", "center"
    )
    if keyboardMinigame["TotalTime"] <= 0:
        return True
    return False
...
def SimonSays(Amount: int, InitialSpeed: int, Time: int):
    ...
    if simonSays["Phase"] == "Initial":
        if simonSays["PhaseInitialTime"] < (simonSays["InitialSpeed"] - 2):
            pyterm.addItem(
                assets[
                    "Grid" + str(simonSays["Order"][simonSays["PhaseInitialFrame"]])
                ],
                0,
                0,
                "center",
        ...
        pyterm.addItem(
            str(simonSays["Order"][simonSays["PlayerFrame"]]),
            0,
            0,
            "bottom left",
            "bottom left",
        ...
        if (
            (GridCenter[0] - 20 <= location[0] <= GridCenter[0] - 1)
            and (GridCenter[1] - 7 <= location[1] <= GridCenter[1] - 1)
            and (MouseDetect.ClickDetect("Left", "On"))
        ):  # y7, x9
...
def BlackHole(Range: float, Strength: float, Unpredictability: int, Time: int):
    ...
    blackholeTextCoords = pyterm.addItem(
        str(assets["BlackHole"]),
        blackHole["Location"][0],
        blackHole["Location"][1],
        "center",
        "center",
    )
    ...
    if (
        math.sqrt(
            (blackholeCoords[0] - mouse.get_position()[0]) ** 2
            + (blackholeCoords[1] - mouse.get_position()[1]) ** 2
        )
        <= Strength
    ):
        ...
        mouse.move(
            (blackholeCoords[0] - mouse.get_position()[0])
            * Strength
            / max(
                math.sqrt(
                    (blackholeCoords[0] - mouse.get_position()[0]) ** 2
            ...
            (blackholeCoords[1] - mouse.get_position()[1])
            * Strength
            / max(
                math.sqrt(
                    (blackholeCoords[0] - mouse.get_position()[0]) ** 2
...
def Shielded(Range: float, Time: int, Unpredictability: int, MaxWait: int):
    ...
    shielded["Time"] -= 1
    if (random.randint(1, Unpredictability) == 1) or (
        (shielded["PastTime"] - shielded["Time"]) >= MaxWait
    ):
        shielded["Location"] = (
            random.randint(-Range[0], Range[0]),
            random.randint(-Range[1], Range[1]),
    ...
    score += (
        math.sqrt(
            (location[0] - shieldedpos[0]) ** 2 + (location[1] - shieldedpos[1]) ** 2
        )
        / 1800
    )
...
def CircleDefend(SpawnRate: int, SpeedRange: float, Time: int):
    ...
    if circleDefend["SpawnRate"] <= 0:
        ...
        while 0 <= spawnlocationx <= os.get_terminal_size().columns:
            spawnlocationx = random.randint(
                round(-0.5 * os.get_terminal_size().columns),
                round(1.5 * os.get_terminal_size().columns),
            )
        while 0 <= spawnlocationy <= os.get_terminal_size().lines:
            spawnlocationy = random.randint(
                round(-0.5 * os.get_terminal_size().lines),
                round(1.5 * os.get_terminal_size().lines),
            )
        circleDefend["Spawns"].append(
            {
                "Location": (spawnlocationx, spawnlocationy),
                "Speed": random.randint(
                    circleDefend["SpeedRange"][0], circleDefend["SpeedRange"][1]
                )
                / 100,
            }
        )
    ...
    for i in circleDefend["Spawns"]:
        i["Location"] = (
            i["Location"][0]
            + (centercoords[0] - i["Location"][0])
            * i["Speed"]
            / max(
                math.sqrt(
                    (centercoords[0] - i["Location"][0]) ** 2
                    + (centercoords[1] - i["Location"][1]) ** 2
                ),
                0.1,
            ),
            i["Location"][1]
            + (centercoords[1] - i["Location"][1])
            * i["Speed"]
            / max(
                math.sqrt(
                    (centercoords[0] - i["Location"][0]) ** 2
        ...
        if (i["Location"][0] - 2 <= location[0] <= i["Location"][0] + 2) and (
            i["Location"][1] - 1 <= location[1] <= i["Location"][1] + 1
        ):
...
def MouseMinigame(Speed: int, Time: int):
    ...
    if (
        (mouseMinigame["CurrentKey"] == "Left")
        or (mouseMinigame["CurrentKey"] == "Not Right")
        or (mouseMinigame["CurrentKey"] == "Not Middle")
    ) and (MouseDetect.ClickDetect("Left", "On")):
        mouseMinigame["Next"] = 0
        mouseMinigame["CurrentKey"] = random.choice(Inputs)
        score += 0.75
        ...
        (mouseMinigame["CurrentKey"] == "Right")
        or (mouseMinigame["CurrentKey"] == "Not Left")
        or (mouseMinigame["CurrentKey"] == "Not Middle")
    ) and (MouseDetect.ClickDetect("Right", "On")):
        mouseMinigame["Next"] = 0
        mouseMinigame["CurrentKey"] = random.choice(Inputs)
        score += 0.75
        time.sleep(0.1)
    elif (
        (mouseMinigame["CurrentKey"] == "Middle")
        or (mouseMinigame["CurrentKey"] == "Not Right")
        or (mouseMinigame["CurrentKey"] == "Not Left")
        or (mouseMinigame["CurrentKey"] == "Not Left Or Right")
    ) and (MouseDetect.ClickDetect("Middle", "On")):
        mouseMinigame["Next"] = 0
        mouseMinigame["CurrentKey"] = random.choice(Inputs)
        score += 0.75
        time.sleep(0.1)
...
def DodgeGrid(
    Character: str, SpeedRange: float, Time: int, SpawnRate: int, InverseBias=10
):
    ...
    pyterm.addItem(
        str(dodgeGrid["Character"]),
        round(dodgeGrid["Location"][0]),
        round(dodgeGrid["Location"][1]),
        "center",
        "center",
    )
    ...
    if dodgeGrid["SpawnRate"] <= 0:
        ...
        if random.randint(1, InverseBias) == 1:
            dodgeGrid["Hurt"].append(
                {
                    "Location": [
                        dodgeGrid["Location"][0],
                        -random.randint(
                            os.get_terminal_size().lines + 1,
                            round(os.get_terminal_size().lines * 1.3),
                        ),
                    ],
                    "Speed": random.randint(SpeedRange[0], SpeedRange[1]) / 100,
                }
            )
        dodgeGrid["Hurt"].append(
            {
                "Location": [
                    (random.randint(-39, 40)),
                    -random.randint(
                        os.get_terminal_size().lines + 1,
                        round(os.get_terminal_size().lines * 1.3),
                    ),
    ...
    for i in dodgeGrid["Hurt"]:
        ...
        pyterm.addItem(
            "X", round(i["Location"][0]), round(i["Location"][1]), "center", "center"
        )
        if (round(dodgeGrid["Location"][0]) == round(i["Location"][0])) and (
            round(dodgeGrid["Location"][1]) == round(i["Location"][1])
        ):
            score -= 0.65
...
def TrackingMinigame(
    Character: int, SpawnRate: int, SpeedRange: float, DespawnRate: int, Time: int
):
    ...
    pyterm.addItem(
        str(trackingMinigame["Character"]),
        round(trackingMinigame["Location"][0]),
        round(trackingMinigame["Location"][1]),
        "center",
        "center",
    )
    ...
    if trackingMinigame["SpawnRate"] <= 0:
        ...
        trackingMinigame["Hurt"].append(
            {
                "Location": [spawnlocationx, spawnlocationy],
                "Speed": random.randint(
                    trackingMinigame["SpeedRange"][0], trackingMinigame["SpeedRange"][1]
                )
                / 100,
                "DespawnRate": trackingMinigame["DespawnRate"],
            }
        )
    for i in trackingMinigame["Hurt"]:
        i["Location"] = [
            i["Location"][0]
            + (trackingMinigame["Location"][0] - i["Location"][0])
            * i["Speed"]
            / max(
                math.sqrt(
                    (trackingMinigame["Location"][0] - i["Location"][0]) ** 2
                    + (trackingMinigame["Location"][1] - i["Location"][1]) ** 2
                ),
                0.1,
            ),
            i["Location"][1]
            + (trackingMinigame["Location"][1] - i["Location"][1])
            * i["Speed"]
            / max(
                math.sqrt(
                    (trackingMinigame["Location"][0] - i["Location"][0]) ** 2
        ...
        if (trackingMinigame["Location"][0] == i["Location"][0]) and (
            trackingMinigame["Location"][1] == i["Location"][1]
        ):
            score -= 1
            trackingMinigame["Hurt"].remove(i)
