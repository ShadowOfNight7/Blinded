...
def AddAttackAim(Range: tuple, Duration: int, Clickable = True):
    ...
    Coords = pyterm.renderLiteralItem("Target1", RandomizerX, RandomizerY, "center", "center")
...
def CircleStay(location, Time: int, Range: tuple, Speed: float, Unpredictability: int, ScoreMulti = 1):
    ...
    circleStay["absoluteloc"] = pyterm.renderLiteralItem(assets["CircleStay"], round(circleStay["location"][0]), round(circleStay["location"][1]), "center", "center")
...
def AimMinigame(location, Speedrange: tuple, Repeats = 1, ScoreMulti = 1):
    ...
    pyterm.renderLiteralItem(assets["AimMinigame"].splitlines()[aimMinigame["CurrentFrame"] - 1], 0, 7, "center", "center")

    if (MouseDetect.ClickDetect("Left", "On") or MouseDetect.GeneralDetect(32, "On")) and (clickbuffer == False):
        score += aimMinigame["CurrentFrame"] / 6 * ScoreMulti
        aimMinigame["Repeats"] -= 1
        aimMinigame["CurrentFrame"] = 0
        aimMinigame["CurrentWait"] = 0
        aimMinigame["Speed"] = random.randint(Speedrange[0], Speedrange[1])    
...
def SimonSays(location, Amount: int, InitialSpeed: int, Time: int, ScoreMulti = 1):
    ...
    if simonSays["Phase"] == "Initial":
        if simonSays["PhaseInitialTime"] < (simonSays["InitialSpeed"] - 2):
            pyterm.renderLiteralItem(assets["Grid" + str(simonSays["Order"][simonSays["PhaseInitialFrame"]])], 0, 0, "center", "center")
        else:
            pyterm.renderLiteralItem(assets["Grid"], 0, 0, "center", "center")
        ...
        GridCenter = pyterm.renderLiteralItem(assets["Grid"], 0, 0, "center", "center")
        pyterm.renderLiteralItem(str(simonSays["Order"][simonSays["PlayerFrame"]]), 0, 0, "bottom left", "bottom left")
        if (GridCenter[0] - 20 <= location[0] <= GridCenter[0] - 1) and (GridCenter[1] - 7 <= location[1] <= GridCenter[1] - 1) and (MouseDetect.ClickDetect("Left", "On")): #y7, x9
            if (simonSays["Order"][simonSays["PlayerFrame"]] == "TL"):
                ...
                pyterm.renderLiteralItem(assets["GridTL"], 0, 0, "center", "center")
                score += 1 * ScoreMulti
            else:
                score = max(score - 1, 0)
            ...
            if (simonSays["Order"][simonSays["PlayerFrame"]] == "BL"):
                simonSays["PlayerFrame"] += 1
                pyterm.renderLiteralItem(assets["GridBL"], 0, 0, "center", "center")
                score += 1 * ScoreMulti
            else:
                score = max(score - 1, 0)
        elif (GridCenter[0] + 20 >= location[0] >= GridCenter[0] + 1) and (GridCenter[1] - 7 <= location[1] <= GridCenter[1] - 1) and (MouseDetect.ClickDetect("Left", "On")): #y7, x9
            if (simonSays["Order"][simonSays["PlayerFrame"]] == "TR"):
                simonSays["PlayerFrame"] += 1
                pyterm.renderLiteralItem(assets["GridTR"], 0, 0, "center", "center")
                score += 1 * ScoreMulti
            else:
                score = max(score - 1, 0)
        elif (GridCenter[0] + 20 >= location[0] >= GridCenter[0] + 1) and (GridCenter[1] + 7 >= location[1] >= GridCenter[1] + 1) and (MouseDetect.ClickDetect("Left", "On")): #y7, x9
            if (simonSays["Order"][simonSays["PlayerFrame"]] == "BR"):
                simonSays["PlayerFrame"] += 1
                pyterm.renderLiteralItem(assets["GridBR"], 0, 0, "center", "center")
                score += 1 * ScoreMulti
            else:
                score = max(score - 1 * ScoreMulti, 0)
...
def BlackHole(location, Range: float, Strength: float, Unpredictability: int, Time: int, ScoreMulti = 1):
    ...
    if random.randint(1, Unpredictability) == 1:
        blackHole["Location"] = (random.randint(-Range[0], Range[0]), random.randint(-Range[1], Range[1]))
    blackholeTextCoords = pyterm.renderLiteralItem(str(assets["BlackHole"]), blackHole["Location"][0], blackHole["Location"][1], "center", "center")
    ...
    # pyterm.renderLiteralItem(str((blackholeCoords[0] - mouse.get_position()[0]) * Strength / math.sqrt((blackholeCoords[0] - mouse.get_position()[0]) ** 2 + (blackholeCoords[1] - mouse.get_position()[1]) ** 2)) + " " + str((blackholeCoords[1] - mouse.get_position()[1]) * Strength / math.sqrt((blackholeCoords[0] - mouse.get_position()[0]) ** 2 + (blackholeCoords[1] - mouse.get_position()[1]) ** 2)), 0, 7, "center", "center")
    score += 13.5 / max(((blackholeCoords[0] - mouse.get_position()[0]) ** 2 + (blackholeCoords[1] - mouse.get_position()[1]) ** 2) ** 0.55, 180) * ScoreMulti
...
def Reaction(location, TimeRange: float, Repetitions: int, ScoreMulti = 1):
    ...
    reaction["Time"] -= 1
    if reaction["Time"] <= 0:
        pyterm.renderLiteralItem(str(assets["ReactionFlash"]), 0, 0, "center", "centre")
        if MouseDetect.ClickDetect("Left", "On"):
            reaction["Repetitions"] -= 1
            reaction["Time"] = random.randint(TimeRange[0], TimeRange[1])
        else:
            score += 0.16 * ScoreMulti
...
def Shielded(location, Range: float, Time: int, Unpredictability: int, MaxWait: int, ScoreMulti = 1):
    ...
    shieldedpos = pyterm.renderLiteralItem(str(assets["Shielded"]), shielded["Location"][0], shielded["Location"][1], "center", "centre")
...
def CircleDefend(location, SpawnRate: int, SpeedRange: float, Time: int, ScoreMulti = 1):
    ...
    if circleDefend["Time"] <= 0:
        ...
        circleDefend["Spawns"] = []
    centercoords = pyterm.renderLiteralItem(str(assets["BlackHole"]), 0, 0, "center", "center")
    ...
    for i in circleDefend["Spawns"]:
        ...
        pyterm.renderLiteralItem("#", round(i["Location"][0]), round(i["Location"][1]))
...
def DodgeGrid(location, Character: str, SpeedRange: float, Time: int, SpawnRate: int, InverseBias = 10, ScoreMulti = 1):
    ...
    for i in dodgeGrid["Hurt"]:
        ...
        pyterm.renderLiteralItem("X", round(i["Location"][0]), round(i["Location"][1]), "center", "center")
        if (round(dodgeGrid["Location"][0]) == round(i["Location"][0])) and (round(dodgeGrid["Location"][1]) == round(i["Location"][1])):
            score += 0.65 * ScoreMulti
...
def TrackingMinigame(location, Character: int, SpawnRate: int, SpeedRange: float, DespawnRate: int, Time: int, ScoreMulti = 1):
    ...
    pyterm.renderLiteralItem(str(trackingMinigame["Character"]), round(trackingMinigame["Location"][0]), round(trackingMinigame["Location"][1]), "center", "center")
    ...
    if trackingMinigame["SpawnRate"] <= 0:
        ...
        spawnlocationx = 1
        spawnlocationy = 1
        while (-39 >= spawnlocationx >= 40) and (-13 >= spawnlocationy >= 13):
            spawnlocationx = random.randint(round(-os.get_terminal_size().columns), round(os.get_terminal_size().columns))
            spawnlocationy = random.randint(round(-os.get_terminal_size().lines), round(os.get_terminal_size().lines))
...
targets = {"Time": 0, "Range": (0, 0), "Duration": 0, "Clickable": True}
...
while False:
    ...
    #     timed = 0
    # score += AttackAim()
    # pyterm.renderLiteralItem(str(score))

    # pyterm.renderLiteralItem(str(location), 0, 0, "top right", "top right")
    # pyterm.renderLiteralItem(str(buttons), 0, 0, "bottom left", "bottom left")
    

    #Circles
 
    # CircleStay((round(os.get_terminal_size().columns / 3), round(os.get_terminal_size().lines / 3)), 10000, 0.5, 25)
    # pyterm.renderLiteralItem(str(circleStay), 0, 0, "bottom left", "bottom left")
    ...
    # pyterm.renderLiteralItem(str(aimMinigame["Repeats"]), 0, 0, "bottom left", "bottom left")
    ...
    timed += 1
    pyterm.renderLiteralItem(str(score))
    pyterm.renderLiteralItem(str(time.time() - FalseTime), 0, 0, "top right", "top right")
